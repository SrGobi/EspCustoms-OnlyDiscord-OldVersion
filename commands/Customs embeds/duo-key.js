...
module.exports = {
    ...
    run: async (discordclient, message, args) => {
        ...
        if(message.member.hasPermission('MANAGE_MESSAGES')){
            const channel = message.guild.channels.cache.find(ch => ch.name === 'duo-key');
            if (!channel)
                return message.channel.send('No puedo encontrar ese canal. Cree un canal llamado duo-key.').then(m => m.delete({timeout: 5000}));
            
            if (args.length < 1)
                return message.channel.send('Debes especificar el codigo de la custom').then(m => m.delete({timeout: 5000}));
                const embed = new Discord.MessageEmbed()
                    .setTitle("Duo Arena Custom")
                    ...
                    .addField("**MODO DE JUEGO**", `ARENA DUO`, true)
                    .addField("**NORMAS:**", `
                ...
                message.guild.channels.cache.find(ch => ch.name === 'duo-key').send('<@&614777211555414017>', embed);
        }else{
